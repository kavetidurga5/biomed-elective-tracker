...
function run() {
  ...
  check("empty-string timestamp is also dropped",
    ctx.selectAlerts([{ Timestamp: "", "Entry Type": "Blocker", Description: "x" }], 6).alerts.length, 0);

  check("non-alert entry types are excluded",
    ctx.selectAlerts([{ Timestamp: "Date(2026,8,10)", "Entry Type": "Progress Update", Description: "x" }], 6).alerts.length, 0);
